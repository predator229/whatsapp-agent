...
describe('MerchantProfile', () => {
  ...
  it('MerchantProfileSchema.parse itself rejects a floor above listed price', () => {
    ...
    if (!result.success) {
      expect(result.error.issues[0].path).toEqual(['negotiation', 'perProduct', 'gari', 'floorPrice'])
    }
  })
  it('MerchantProfileSchema.parse rejects a duplicate productId with a path to the catalogue entry', () => {
    ...
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].path).toEqual(['catalogue', pilotProfile.catalogue.length, 'productId'])
    }
  })
  it('MerchantProfileSchema.parse rejects an unknown productId in quantityDiscounts', () => {
    const bad = { ...pilotProfile, negotiation: { ...pilotProfile.negotiation,
      quantityDiscounts: [...pilotProfile.negotiation.quantityDiscounts, { productId: 'ghost', minQuantity: 2, percentOff: 10 }] } }
    const result = MerchantProfileSchema.safeParse(bad)
    expect(result.success).toBe(false)
    if (!result.success) {
      expect(result.error.issues[0].path).toEqual(['negotiation', 'quantityDiscounts', 1, 'productId'])
    }
  })
})
