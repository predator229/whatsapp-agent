...
function rowsFor(p: Product): Row[] {
  ...
  switch (p.pricing.kind) {
    case 'unit':
      return p.pricing.variants.map((v) => ({
        ...base,
    ...
    case 'bulk':
      return [
        {
          ...base,
          price: p.pricing.pricePerUnit,
          unitOfMeasure: p.pricing.unitOfMeasure,
          stock: p.pricing.stockQuantity,
        },
      ]
    case 'madeToOrder':
      return [
        {
          ...
          leadTimeDays: p.pricing.leadTimeDays,
          depositPercent: p.pricing.depositPercent,
        },
      ]
  }
}
...
export function exportProfile(profile: MerchantProfile): {
  catalogueCsv: string
  profileJson: string
} {
  const lines = profile.catalogue
    .flatMap(rowsFor)
    .map((row) => CATALOGUE_CSV_COLUMNS.map((c) => csvCell(row[c])).join(','))
  return {
