...
describe('ProductSchema', () => {
  ...
  it('rejects a unit product without variants', () => {
    expect(() =>
      ProductSchema.parse({ ...base, pricing: { kind: 'unit', variants: [] } }),
    ).toThrow()
  })
  it('accepts bulk and made-to-order', () => {
    const bulk = ProductSchema.parse({
      ...base,
      pricing: {
        kind: 'bulk',
        unitOfMeasure: 'kg',
  ...
  it('rejects non-integer or negative prices', () => {
    ...
    expect(() =>
      ProductSchema.parse({
        ...
        pricing: {
          kind: 'unit',
          variants: [{ variantId: 'v', label: 'x', attributes: {}, price: -1, stock: null }],
        },
      }),
    ).toThrow()
  })
  ...
  it('rejects a non-integer variant stock', () => {
    expect(() =>
      ProductSchema.parse({
        ...
        pricing: {
          kind: 'unit',
          variants: [{ variantId: 'v', label: 'x', attributes: {}, price: 1000, stock: 1.5 }],
        },
      }),
    ).toThrow()
  })
